...
public class MLPClassifier {
    ...
    public static void main(String[] args) throws Exception {
       ...
       /* RecordReader rr = new CSVRecordReader();
        ...
        train.initialize(new FileSplit(new File("dl4j_0.4_examples/src/main/resources/classification/rgb_data_3_train.csv")));
        DataSetIterator trainIter = (DataSetIterator) new RecordReaderDataSetIterator(train,batchSize_train,0,3);
        ...
        DataSetIterator testIter = (DataSetIterator) new RecordReaderDataSetIterator(test,batchSize_test,0,3);
        ...
        OutputStream fos = Files.newOutputStream(Paths.get("coeff_test.bin"));
        DataOutputStream dos = new DataOutputStream(fos);
        Nd4j.write(model.params(), dos);
        dos.flush();
        dos.close();
        FileUtils.writeStringToFile(new File("test.json"), model.getLayerWiseConfigurations().toJson());

        MultiLayerConfiguration confFromJson = MultiLayerConfiguration.fromJson(FileUtils.readFileToString(new File("test.json")));
        DataInputStream dis = new DataInputStream(new FileInputStream("coeff_test.bin"));
